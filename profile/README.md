# Tech of Our Own

We build **non-extractive consumer technology**: local-first, user-owned, repairable, and auditable.

**Non‑negotiables:** no subscriptions, no surveillance business models, no hostage mechanics.

---

## Start here (new to the org?)

- **Mission:** https://github.com/techofourown/org-techofourown/blob/main/docs/policies/founding/MISSION.md
- **Vision:** https://github.com/techofourown/org-techofourown/blob/main/docs/policies/founding/VISION.md
- **Values:** https://github.com/techofourown/org-techofourown/blob/main/docs/policies/founding/VALUES.md
- **Constitution (how governance + economics work):**
  https://github.com/techofourown/org-techofourown/blob/main/docs/policies/founding/CONSTITUTION.md

---

## The OurBox stack (repo map)

### Organization / governance (source of truth)
- **org-techofourown** — founding docs, policies, ADRs/RFCs, org-level tooling
  https://github.com/techofourown/org-techofourown

### Software
- **sw-ourbox-os** — OurBox OS (apps, decisions, spec)
  https://github.com/techofourown/sw-ourbox-os
- **sw-ourbox-installer** — start here to prepare install media for OurBox OS on supported devices
  https://github.com/techofourown/sw-ourbox-installer

### Hardware
- **pf-ourbox** — OurBox product family (Matchbox + Tinderbox hardware)
  https://github.com/techofourown/pf-ourbox

### Images / build artifacts
- **img-ourbox-matchbox** — build + publish bootable images for OurBox Matchbox (RPi)
  https://github.com/techofourown/img-ourbox-matchbox
- **img-ourbox-tinderbox** — build + publish bootable workflow for OurBox Tinderbox (NVIDIA Jetson)
  https://github.com/techofourown/img-ourbox-tinderbox
- **img-ourbox-woodbox** — build + publish bootable installer media for OurBox Woodbox (x86 mini-PC / Ubuntu Server)
  https://github.com/techofourown/img-ourbox-woodbox

### Website
- **web-techofourown** — website content
  https://github.com/techofourown/web-techofourown

---

## How we work

- **Decisions live in ADRs**, exploration lives in RFCs.
  Start here: https://github.com/techofourown/org-techofourown/blob/main/docs/README.md

- **Be kind, be rigorous, don't be extractive.**
  Code of Conduct: https://github.com/techofourown/.github/blob/main/CODE_OF_CONDUCT.md
  Contributing: https://github.com/techofourown/.github/blob/main/CONTRIBUTING.md

---

## Licensing posture (plain language)

- **Core software**: AGPLv3 (network users must be able to get the exact source)
  https://github.com/techofourown/org-techofourown/blob/main/docs/decisions/ADR-0001-adopt-agplv3-core-software.md

- **Core hardware designs**: CERN-OHL-S-2.0 (strong reciprocity for shipped modifications)
  https://github.com/techofourown/org-techofourown/blob/main/docs/decisions/ADR-0005-adopt-cern-ohl-s-2-0-core-hardware-designs.md

- **Public documents and educational media**: CC BY 4.0 (reuse, adaptation, and commercial use are allowed with attribution and change indication)
  https://github.com/techofourown/org-techofourown/blob/main/docs/decisions/ADR-0006-adopt-cc-by-4-0-for-public-documents-and-educational-media.md
